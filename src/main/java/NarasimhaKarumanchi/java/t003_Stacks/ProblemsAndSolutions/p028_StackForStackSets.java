...
public class p028_StackForStackSets<T> implements p027_CreateNewStackWhenOldStackOverflows<T> {
	...
	public p028_StackForStackSets(int capacity) {
...
class StackSets<T> {
	...
	private List<p028_StackForStackSets<T>> listOfStacks;
	...
	// get the last stack
	public p028_StackForStackSets<T> getLastStack() {
	...
	// get the nth stack
	public p028_StackForStackSets<T> getNthStack(int n) {
		int size = listOfStacks.size();
		if (size <= 0) {
			return null;
		}
	...
	// push to stack
	public void push(T data) {
		p028_StackForStackSets<T> lastStack = this.getLastStack();
		if (lastStack == null) {
			lastStack = new p028_StackForStackSets<>(threshold);
			lastStack.push(data);
			...
			if(!lastStack.isAtCapacity()) {
				...
				p028_StackForStackSets<T> newLastStack = new p028_StackForStackSets<>(threshold);
	...
	// pop from nth stack
	public T pop(int n) {
		p028_StackForStackSets<T> nthStack = this.getNthStack(n);
		T data = nthStack.pop();
		if (nthStack.size() == 0) {
	...
	public String toString() {
		...
		for (int i = 0; i < listOfStacks.size(); i++) {
			p028_StackForStackSets<T> stack = listOfStacks.get(i);
			int j = i;
